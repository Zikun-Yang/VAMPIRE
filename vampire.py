...
# Self-defined class
from stringDecompose import Decompose       # Decompose and annotate sequences
from estimateParameters import Estimate    # Automatically find proper parameters

def find_N(task):
    """
    Identifies and returns the coordinates of 'N' characters in a given sequence.
    
    Args:
        task (tuple): Contains the task ID (pid), total number of tasks (total_task), and the sequence to process.
        
    Returns:
        pd.DataFrame: A DataFrame containing the start and end coordinates of 'N' regions.
    """
    ...
    if len(n_indices) == 0:
        if not args_dict['Output Options']['quiet']:
            console.print(f'Process N Complete: {pid}/{total_task}', style="white")
    ...
    N_coordinate_df[['start', 'end']] += (pid - 1) * window_length
...
def decompose_sequence(task):
    seq_name, pid, total_task, sequence, ksize= task
    seg = Decompose(sequence, ksize, args_dict['Decomposition Options'], args_dict['Annotation Options'])
    if not args_dict['Decomposition Options']['no_denovo']:
        seg.count_kmers()
        seg.build_dbg(args_dict['I/O Options']['output'], seq_name, pid)
        seg.find_motif()
    if not args_dict['Output Options']['quiet']:
        console.print(f'Decomposition Complete: {pid}/{total_task}', style="white")
    return ( seq_name, pid, total_task, seg )

def single_motif_annotation(task):
    seq_name, pid, total_task, seg = task
    seg.annotate_with_motif()
    # polish 5' end annotation
    ###if pid == 1:
    ###    seg.annotate_polish_head()
    df = seg.annotation
    df[['start', 'end']] += (pid - 1) * step_length
    df = df.sort_values(by='end').reset_index(drop=True)
    df.to_csv(f"{args_dict['I/O Options']['output']}_temp/{seq_name}_anno_{pid}.csv", index = False)
    if not args_dict['Output Options']['quiet']:
        console.print(f'Single Motif Annotation Complete: {pid}/{total_task}', style="white")
...
def rc(seq):
    if args_dict['General Options']['alphabet'] == 'ATCG':
        complement = {'A': 'T', 'T': 'A', 'C': 'G', 'G': 'C', 'N':'N'}
    else:
        complement = {chr(i): chr(90 - (i - 65)) for i in range(65, 91)}    # 'A':'Z', 'B':'Y'
...
class ArgumentParser:
    def __init__(self, script_directory):
        ...
        script_directory: The directory where the script is located, used to set default paths.
        """
    ...
    def _create_parser(self):
        ...
        parser = argparse.ArgumentParser(
            description='TRkScan v1.0\n'
                        'Usage: python trkscan.py [--auto] [options] [input.fa] [output_prefix]\n'
                        'For example: python trkscan.py --auto [input.fa] [output_prefix]\n'
                        '             python trkscan.py -k 13 -s 15 [input.fa] [output_prefix]\n',
        ...
        general_group.add_argument('-c', '--config', type=str, default=None, help='config file in JSON, CONFLICT with --AUTO [None]')
        general_group.add_argument('--alphabet', type=str, choices= ['ATCG', 'ABCD'], default='ATCG', help='select valid alphabet in FASTA [ATCG]')
        ...
        general_group.add_argument('-w', '--window-length', type=int, default=5000, help='parallel window size [5000]')
        general_group.add_argument('-o', '--overlap-length', type=int, default=1000, help='windows overlap size [1000]')
        general_group.add_argument('-r', '--resource', type=int, default=50, help='memory limit (GB) [50]')

        # Decomposition Options
        decompose_group = parser.add_argument_group('Decomposition Options')
        decompose_group.add_argument('-k', '--ksize', type=int, default=5, help='k-mer size for building De Bruijn graph [5]')
        decompose_group.add_argument('-m', '--motif', type=str, default=f'{self.script_directory}/data/refMotif.fa', help='reference motif set path')
        decompose_group.add_argument('-n', '--motifnum', type=int, default=30, help='maximum number of motifs [30]')
        decompose_group.add_argument('--abud-threshold', type=float, default=0.01, help='min threshold campared with top edge weight [0.01]')
        decompose_group.add_argument('--abud-min', type=int, default=3, help='min edge weight in De Bruijn graph [3]')
        decompose_group.add_argument('--plot', action='store_true', help='paint De bruijn graph for each windows [False]')
        decompose_group.add_argument('--no-denovo', action='store_true', help='not de novo find motifs, use reference motifs to annotate [False]')

        # Annotation Options
        annotation_group = parser.add_argument_group('Annotation Options')
        annotation_group.add_argument('--force', action='store_true', help='add reference motifs into annotation [False]')
        annotation_group.add_argument('-a', '--annotation-dist-ratio', type=float, default=0.4, help='max distance to map\n= 0.4 * length of motif [0.4]')
        annotation_group.add_argument('-f', '--finding-dist-ratio', type=float, default=0.2, help='max distance to query in reference motif set\n= 0.2 * length of motif [0.4]')
        annotation_group.add_argument('--match-score', type=float, default=1, help='score per matched base [1]')
        annotation_group.add_argument('--lendif-penalty', type=float, default=0.01, help='hope that mapped sequence is as long as the motif [0.01]')
        ...
        annotation_group.add_argument('--perfect-bonus', type=float, default=0.5, help='bonus per matched base if there is no difference in alignment [0.5]')

        # Output Options
        output_group = parser.add_argument_group('Output Options')
        output_group.add_argument('--quiet', action='store_true', help="don't output thread completion informing")
        output_group.add_argument('--file', nargs='+', default=['setting', 'concise', 'annotation', 'dist', 'motif'], help='minimum output score [setting concise annotation dist motif]')
    ...
    def _post_process_args(self):
        ...
        self.args.input = self.args.input.replace('\\', '/')
        self.args.output = self.args.output.replace('\\', '/')

        # Create the output directory
        os.makedirs(f'{self.args.output}_temp', exist_ok=True)

        # Calculate the step size
        self.args.step_length = self.args.window_length - self.args.overlap_length

    def get_args_dict(self):
        """
        Convert the parsed arguments into a dictionary format.
        """
        if self.args is None:
            raise ValueError("Please call parse_args() method to parse arguments first.")
        
        if self.args.AUTO and self.args.config is not None:
            raise ValueError("--config conflicts with --AUTO")
        ...
        if self.args.config is None:
            args_dict = {
                    ...
                    "Decomposition Options": {
                        ...
                        "plot": self.args.plot,
                        "no_denovo": self.args.no_denovo
                    },
                    "Annotation Options": {
                        "force": True if self.args.no_denovo else self.args.force,
                        ...
                        "lendif_penalty": self.args.lendif_penalty,
                        "gap_penalty": self.args.gap_penalty,
                        "distance_penalty": self.args.distance_penalty,
                        "perfect_bonus": self.args.perfect_bonus
                    },
                    "Output Options": {
            ...
            if self.args.AUTO:
                ...
                ksize_list = [3, 5, 9, 11, 15, 31, 41, 71]
            ...
            args_dict = json.loads(self.args.config)
    ...
    def get_args_json(self):
        """
        Convert the parsed arguments into JSON format.
        """
        import json
        return json.dumps(self._args_dict, indent=4)
...
# script_directory
trkscan_parser = ArgumentParser(script_directory)
args = trkscan_parser.parse_args()

# get parameters in dictionary format
args_dict = trkscan_parser.get_args_dict()

# get parameters in JSON format
args_json = trkscan_parser.get_args_json()
with open(f"{args_dict['I/O Options']['output']}.setting.json", 'w') as setting:
...
if __name__ == "__main__":
    ...
    ####################################################################
    # Read data and reference motif set, construct BK tree
    ####################################################################
    ...
    with open(args_dict['Decomposition Options']['motif'], 'r') as handle:
    ...
    for record in motif_records:
        ...
        motif_rc = rc(motif)    # add inverted motif
        tree.add(motif_rc)
    ...
    for record in seq_records:
        ...
        step_length = args_dict['General Options']['step_length']

        total_task = max(1, (seqLenwN - window_length) // step_length + 1)
        
        tasks = [(cur + 1, total_task, seq[cur * step_length : (cur + 1) * step_length]) for cur in range(total_task)]
        ...
        with Pool(processes = args_dict['General Options']['thread']) as pool:
            ...
            while cur < total_task:
                tasks.append(tuple([seq_name,
                                    ...
                                    args_dict['Decomposition Options']['ksize']]))
                cur += 1
        ...
        ################################################################
        if not args_dict['Decomposition Options']['no_denovo']:
            motif_df_list = [result.motifs for _, _, _, result in results]
        ...
        idx = 0
        current_pid = 1
        df = pd.read_csv(f"{args_dict['I/O Options']['output']}_temp/{seq_name}_anno_1.csv")
        ...
        for i in range(1, length + 1):
            ...
            if pid > current_pid:
                current_pid = pid
                idx = 0
                df = pd.read_csv(f"{args_dict['I/O Options']['output']}_temp/{seq_name}_anno_{pid}.csv")
                anno_start = df['start'].values
                anno_end = df['end'].values
                anno_motif = df['motif'].values
                anno_distance = df['distance'].values
        ...
        console.print(f'### Start Outputing ###', style="yellow")
    ...
    merged_annotation.to_csv(f"{args_dict['I/O Options']['output']}.concise.tsv", sep = '\t', index = False)
    ...
    console.print('output concise.tsv is done!', style="green")

    if args_dict['General Options']['debug']:
        start_time = time.time()

    if not args_dict['General Options']['debug']:
        subprocess.run(f"rm -r {args_dict['I/O Options']['output']}_temp", shell = True)
    ...
    for record in seq_records:
        ...
        for index, row in tmp.iterrows():
            ...
            while j < len(cigar_string):
                ...
                if symbol == '/':
                    detail_annotation.append([seq_name, seqLen, start, start + length, row.motif, dir, actual_motif, sub_cigar])
                    start = start + length
    ...
    detailed_df = pd.DataFrame(data = detail_annotation, columns = ['seq','length','start','end','motif','dir','actual_motif','CIGAR'])
    detailed_df.to_csv(f"{args_dict['I/O Options']['output']}.annotation.tsv", sep = '\t', index = False)

    if args_dict['General Options']['debug']:
        end_time = time.time()
        print(f"make annotation.tsv: {round(end_time - start_time, 2)} s")

    console.print('output annotation.tsv is done!', style="green")
    ...
    motif_df.to_csv(f"{args_dict['I/O Options']['output']}.motif.tsv", sep = '\t', index = True, index_label='id')
    ...
    console.print('output motif.tsv is done!', style="green")
    ...
    distance_df.to_csv(f"{args_dict['I/O Options']['output']}.dist.tsv", sep = '\t', index = False, columns = ['ref','query','dist','is_rc'])

    if args_dict['General Options']['debug']:
        end_time = time.time()
        print(f"make dist.tsv: {round(end_time - start_time, 2)} s")

    console.print('output dist.tsv is done!', style="green")
