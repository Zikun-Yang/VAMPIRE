...
def calculate_edit_distance_between_motifs(m1: np.ndarray, m2: np.ndarray) -> int:
    ...
    end_state: int = int(np.argmax(score_array[len(m1) - 1, :]))  # 0 -> M, 1 -> I, 2 -> D
    best_j: int = band_argmax_j[len(m1) - 1, end_state]
    ...
    with open(output_filepath, 'w', newline='', encoding='utf-8') as fo:
        ...
        with open(polished_rgn_path, 'r', newline='', encoding='utf-8') as fi:
            ...
            for row in reader:
                ...
                max_score: int = 0
                # score, ops (refined alignment), end_i, end_j, encoded_refined_motif
                candidates: List[Tuple[int, List[str], int, int, np.ndarray]] = []
                encoded_seq = encode_seq_to_array(rgn_dict["sequence"])
                seq_len = len(encoded_seq)
                for motif in rgn_dict["motifs"]:
                    encoded_motif = encode_seq_to_array(motif)
                    print(f"encoded_motif: {decode_array_to_seq(encoded_motif)}") # TODO
                    # first pass: refined motif via Hirschberg (ops, profile, refined_motif)
                    _, _, encoded_refined_motif = hirschberg(encoded_seq, encoded_motif)
                    print(f"encoded_refined_motif: {decode_array_to_seq(encoded_refined_motif)}") # TODO
                    # check the periodicity of the refined motif
                    is_periodic, period = is_periodic_seq(encoded_refined_motif)
                    if is_periodic:
                        encoded_refined_motif = encoded_refined_motif[:period]  # minimal period
                    if len(encoded_refined_motif) == 0:
                        continue

                    # second pass: score from M-row of forward DP; ops from Hirschberg on refined motif
                    M2, _, _ = hirschberg_forward_dp(encoded_seq, encoded_refined_motif)
                    print(f"M2: {M2}") # TODO
                    score = int(M2.max())
                    end_i = seq_len  # 1-based
                    end_j = int(M2.argmax())
                    ha2_ops, _, _ = hirschberg(encoded_seq, encoded_refined_motif)
                    ###print(f"ha2_ops: {ha2_ops}") # TODO
                    if score >= max_score * SECONDARY_SCORE_RATIO:
                        print(f"score: {score}, max_score: {max_score}") # TODO
                        if score > max_score:
                            candidates = [
                                c for c in candidates if c[0] >= score * SECONDARY_SCORE_RATIO
                            ]
                        candidates.append((score, ha2_ops, end_i, end_j, encoded_refined_motif))
                ...
                for candidate in candidates:
                    score, ops, end_i, end_j, encoded_motif = candidate
                    if score == cur_score:
                        continue
                    cur_score = score
                    # start_i / start_j: same convention as traceback_banded_roll_motif (cyclic motif)
                    start_i, start_j = roll_start_from_ops(
                        ops, end_i, end_j, len(encoded_motif)
                    )
...
def traceback_banded_roll_motif(
    ...
    Inputs:
        ...
        start_state: int, DP state at (best_i, best_j) where the traceback begins: 0=M, 1=I, 2=D
    Outputs:
...
def roll_start_from_ops(
    ...
    """
    Recover (start_i, start_j) consistent with traceback_banded_roll_motif by undoing
    ops in reverse order with cyclic motif index (same j-step rules as that traceback).
    """
    i = end_i
    j = end_j
...
def hirschberg_forward_dp(seq: np.ndarray, motif: np.ndarray):
    ...
    for i in range(1, n + 1):
        ...
        for j in range(m):
            # Cyclic motif: previous column is m-1 when j==0 (same as roll_start_from_ops / traceback).
            prev_j_pr = (j - 1 + m) % m if m > 0 else 0
            prev_j_sr = j - 1  # same row, linear; j==0 wrapped after loop

            s = MATCH_SCORE if si == motif[j] else -MISMATCH_PENALTY

            # ---- M ---- (diagonal from previous row, cyclic j)
            best = M_prev[prev_j_pr]
            if I_prev[prev_j_pr] > best:
                best = I_prev[prev_j_pr]
            if D_prev[prev_j_pr] > best:
                best = D_prev[prev_j_pr]
            M_curr[j] = best + s
            ...
            # ---- D ---- (same row, linear j>=1; j==0 uses wrap from m-1 below)
            if prev_j_sr >= 0:
                open_d = M_curr[prev_j_sr] - GAP_OPEN_PENALTY
                ext_d = D_curr[prev_j_sr] - GAP_EXTEND_PENALTY
                D_curr[j] = open_d if open_d > ext_d else ext_d

        # Cyclic wrap: D at j=0 from M_curr[m-1], D_curr[m-1] (same row; not available when j=0 runs).
        if m > 0:
            open_d = M_curr[m - 1] - GAP_OPEN_PENALTY
            ext_d = D_curr[m - 1] - GAP_EXTEND_PENALTY
            d_new = open_d if open_d > ext_d else ext_d
            if d_new > D_curr[0]:
                D_curr[0] = d_new
...
@numba.njit
def rolling_affine_full_dp_trace(seq: np.ndarray, motif: np.ndarray):
    """
    Full (n+1)*m affine DP with cyclic motif, same recurrence as ``hirschberg_forward_dp``,
    plus traceback pointers for M/I/D. Used to recover ops; linear Hirschberg split on motif
    prefix/suffix is wrong for rolling alignment.
    """
    NEG_INF = -10 ** 9
    n = seq.shape[0]
    m = motif.shape[0]
    ...
    I = np.full((n + 1, m), NEG_INF, np.int32)
    D = np.full((n + 1, m), NEG_INF, np.int32)
    trace_M = np.zeros((n + 1, m), np.int8)
    trace_I = np.zeros((n + 1, m), np.int8)
    trace_D = np.zeros((n + 1, m), np.int8)
    ...
    for i in range(1, n + 1):
        ...
        for j in range(m):
            ...
            if prev_j_sr >= 0:
                open_d = M[i, prev_j_sr] - GAP_OPEN_PENALTY
                ext_d = D[i, prev_j_sr] - GAP_EXTEND_PENALTY
                if open_d > ext_d:
                    D[i, j] = open_d
                    trace_D[i, j] = 0
                else:
        ...
        if m > 0:
            open_d = M[i, m - 1] - GAP_OPEN_PENALTY
            ext_d = D[i, m - 1] - GAP_EXTEND_PENALTY
            if open_d > ext_d:
                d_new = open_d
                td = 0
            else:
    ...
    M_last = np.empty(m, dtype=np.int32)
    I_last = np.empty(m, dtype=np.int32)
    D_last = np.empty(m, dtype=np.int32)
    for j in range(m):
        M_last[j] = M[n, j]
        I_last[j] = I[n, j]
        D_last[j] = D[n, j]

    return trace_M, trace_I, trace_D, M_last, I_last, D_last
...
def refined_motif_from_profile(profile: np.ndarray) -> np.ndarray:
    """
    Consensus base per motif column: argmax over A,C,G,T only (columns 0--3).

    Column 4 counts insertions at that motif phase; it must not compete with bases
    in argmax, and must not cause the column to be dropped. Dropping rows where
    argmax was 4 shrinks the period to near-empty motifs (e.g. a single ``A``).
    """
    if profile.size == 0:
        return np.array([], dtype=np.int8)
    bases = profile[:, :4]
    return bases.argmax(axis=1).astype(np.int8)

def _hirschberg_ops(seq: np.ndarray, motif: np.ndarray) -> List[str]:
    """
    Rolling cyclic affine alignment: full DP + traceback.

    Classic Hirschberg splits the motif into a linear prefix and suffix; that matches
    ``F[mid][j] + B[mid][j]`` only for **linear** B. Here ``hirschberg_forward_dp`` is
    **cyclic** (phase j, many wraps), so divide-and-conquer on ``motif[:k]`` /
    ``motif[k:]`` is invalid and was producing wrong ops and profiles.
    """
    if len(seq) == 0:
    ...
    trace_M, trace_I, trace_D, M_last, I_last, D_last = rolling_affine_full_dp_trace(
        seq, motif
    )
    n = len(seq)
    m = len(motif)
    ...
    for j in range(m):
        for s in (0, 1, 2):
            v = M_last[j] if s == 0 else (I_last[j] if s == 1 else D_last[j])
            if v > best:
                best = v
                best_j = j
                best_state = s
...
def hirschberg(seq: np.ndarray, motif: np.ndarray) -> Tuple[List[str], np.ndarray, np.ndarray]:
    """
    Rolling cyclic affine alignment (full DP + traceback); builds profile and refined motif from ops.

    Ops are **not** from linear Hirschberg divide-and-conquer (invalid for cyclic motif DP).
    Inputs:
        seq: np.ndarray, sequence
        motif: np.ndarray, motif
    Outputs:
        ...
        refined: np.ndarray, Length m; per-column consensus argmax on A,C,G,T (insertion counts excluded)
    """
